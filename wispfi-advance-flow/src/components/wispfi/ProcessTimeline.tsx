...
export const ProcessTimeline: React.FC<ProcessTimelineProps> = ({ onOpenModal }) => {
  ...
  const steps = [
    {
      icon: FileText,
      title: t("components.processTimeline.step1Title"),
      text: t("components.processTimeline.step1Text"),
    },
    {
      icon: CheckCircle2,
      title: t("components.processTimeline.step2Title"),
      text: t("components.processTimeline.step2Text"),
    },
    {
      icon: Banknote,
      title: t("components.processTimeline.step3Title"),
      text: t("components.processTimeline.step3Text"),
    },
  ];
  ...
  return (
    <section className="container py-16" style={{ marginTop: "13rem" }}>
        ...
        {steps.map(({ icon: Icon, title, text }, idx) => (
          <li key={title} className="relative rounded-lg border p-6 bg-card">
            ...
            <p className="font-semibold mb-1">
              {t("components.processTimeline.stepLabel", { number: idx + 1 })} — {title}
            </p>
      ...
      <div className="text-center mt-8">
        ...
        <p className="text-xs text-muted-foreground mt-2">⏰ {t("components.processTimeline.processingTime")}</p>
      </div>
    </section>
  );
};
